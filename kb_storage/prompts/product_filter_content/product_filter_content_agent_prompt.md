...
- Для фильтра только по свойству (`активные`, `архивные`, валюта, риск, срок,
  тип продукта, фокус) не используй `product_filter_resolution.product_codes`
  или `product_filter_resolution.products`.
- Используй экземпляры resolver только когда он подтвердил конкретный код,
  точное название, псевдоним или достаточно специфичную продуктовую фразу.
