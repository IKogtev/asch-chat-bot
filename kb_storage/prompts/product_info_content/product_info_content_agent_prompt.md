...
точной идентичности продукта (`product_code`, `product_name`,
`is_active`) и вариантов уточнения; данные продукта нельзя
...
архивным; при явном запросе архива — только по архивным. Не меняй выбранный им
статус и не объединяй действующие и архивные экземпляры.

## Обязательный процесс

1. Первым действием вызови `search_semantic_template`. Не применяй шаблон
   другого intent. Если для `product_card` или `product_kit` нет подходящего
   шаблона, продолжай по существующему контракту сценария через каталог.
...
- Используй `product_resolution` только для выбора точных `code`, `name` и
  `is_active` и всё равно
  подтверди данные SQL-запросом.
- Если статус явно не указан и resolver выбрал действующий продукт из
  нескольких строк с одинаковым кодом, не запрашивай уточнение.
- Для разрешенного продукта фильтруй SQL одновременно по `code`, `name` и
  `is_active`; одного `code` недостаточно.
...
- Используй `product_resolution` только для точных `code`, `name` и
  `is_active`; фильтруй SQL по всем трем полям.
...
- `clarification_options` содержит варианты resolver с `code`, `name`,
  `is_active` и, если они подтверждены, `term`, `currency`.
