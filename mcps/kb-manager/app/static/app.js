...
// app.js - добавить функции после sendNews()
...
async function loadPromptHistory() {
    ...
    try {
        ...
        if (promptHistory.length === 0) {
            historyList.innerHTML = '<div class="empty-state">Нет сохранённых бэкапов</div>';
            return;
        }
        
        historyList.innerHTML = promptHistory.map((item, index) => `
            <div class="prompt-history-item ${item.is_backup ? 'backup-item' : ''}" 
                 onclick="loadPromptFromFile('${item.name}')">
                ...
                <div class="actions" onclick="event.stopPropagation()">
                    <button class="btn-restore" onclick="restorePrompt('${item.name}')">
                        ↩️ Восстановить
                    </button>
                    <button class="btn-delete" onclick="deleteBackup('${item.name}')">
                        🗑️
                    </button>
                </div>
                ` : ''}
        ...
        historyList.innerHTML = `<div class="error">Ошибка: ${err.message}</div>`;
...
// Загрузка промпта из файла истории
async function loadPromptFromFile(filename) {
    const editor = document.getElementById("prompt-editor");
    
    try {
        const res = await fetch(`/api/prompts/history/${encodeURIComponent(filename)}`);
        const data = await res.json();
        ...
        event.target.closest(".prompt-history-item").classList.add("active");
...
async function reloadAgent() {
    ...
    try {
        const res = await fetch("http://adk-agent:8000/api/prompts/reload", {
            method: "POST",
            timeout: 5000
        });
        ...
        if (res.ok) {
            resultDiv.className = "result-message success";
            resultDiv.innerHTML = "✅ Агент перезагружен! Новый промпт активен.";
        } else {
...
// Обновить showTab для вкладки prompts
function showTab(tabName) {
    // ... существующий код ...
