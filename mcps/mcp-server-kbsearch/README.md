...
|------------|----------|---------|
| `MCP_TOKEN` | Токен авторизации для MCP | -REMOVED_SECRET |
...
curl -X POST http://localhost:7001/kbsearch/mcp \
  -H "X-Redacted-Auth: Bearer REMOVED_SECRET" \
...
curl -H "X-Redacted-Auth: Bearer REMOVED_SECRET" \
