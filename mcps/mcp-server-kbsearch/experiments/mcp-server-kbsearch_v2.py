...
EMBEDDING_API_KEY = os.getenv("EMBEDDING_API_KEY", "REMOVED_SECRET")
...
# пути выгрузки файлов: 
EXTERNAL_BASE_URL = os.getenv("EXTERNAL_BASE_URL", f"http://{MCP_HOST}:{MCP_PORT}")
DOWNLOAD_PREFIX = "/kb/download"
    ...
    async with kb_lock:
        ...
        try:
            ...
            for i, node in enumerate(nodes[:top_k], 1):
                # Получаем путь из метаданных
                rel_path = node.metadata.get("source", "unknown")
                result = {
                    "rank": i,
                    "score": node.score,
                    "content": node.get_content()[:500],
                    "file_path": rel_path,
                    "download_url": f"{EXTERNAL_BASE_URL}{DOWNLOAD_PREFIX}/{rel_path}"
                }
...
@mcp.tool()
async def get_file_link(rel_path: Annotated[str, "Relative path to file from KB root"]) -> str:
    """
    Generates a direct download link for a file.
    Use this when you find a document via kb_search and want to give the user a link.
    """
    # Очищаем путь от возможных начальных слешей
    clean_path = rel_path.lstrip("/")
    return f"{EXTERNAL_BASE_URL}{DOWNLOAD_PREFIX}/{clean_path}"

@mcp.tool()
async def list_documents(folder: str = "") -> Dict:
    """
    Returns information about documents available show folders
    """
    base_path = KB_DOCUMENTS_DIR / folder
    ...
    for p in base_path.iterdir():
        files.append({
            "name": p.name,
            "is_dir": p.is_dir(),
            "relative_path": str(p.relative_to(KB_DOCUMENTS_DIR)),
            "download_url": f"{EXTERNAL_BASE_URL}{DOWNLOAD_PREFIX}/{p.relative_to(KB_DOCUMENTS_DIR)}" if p.is_file() else None
        })

    return {"success": True, "items": files}

@mcp.tool()
async def sync_kb_with_filesystem() -> Dict:
    """
    Forces the KB to re-scan the shared folder and update the search index.
    Use this after files were added/moved/deleted via FileSystem MCP.
    """
    logger.info("Ручной запуск синхронизации KB с файловой системой")
    success = await background_rebuild_index()
    return {
        "success": success,
        "message": "Index updated from filesystem" if success else "Sync failed"
    }
...
app = Starlette(
    routes=[
        ...
        # Раздача файлов из той же папки документов
        Mount(DOWNLOAD_PREFIX, app=StaticFiles(directory=str(KB_DOCUMENTS_DIR)), name="download"),
        # REST:
