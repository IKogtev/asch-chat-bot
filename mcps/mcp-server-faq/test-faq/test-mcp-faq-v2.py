...
MCP_TOKEN = os.getenv("MCP_TOKEN", "REMOVED_SECRET-here")
...
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY", "REMOVED_SECRET")
...
S3_ACCESS_KEY = os.getenv("S3_ACCESS_KEY", "REMOVED_SECRET")
S3_SECRET_KEY = os.getenv("S3_SECRET_KEY", "REMOVED_SECRET")
...
async def test_faq_indexing() -> Dict[str, Any]:
    ...
    """
    logger.info("\n" + "=" * 70)
    logger.info("2. ИНДЕКСАЦИЯ БЗ: ЗАГРУЗКА И ИНДЕКСИРОВАНИЕ ДОКУМЕНТОВ")
    logger.info("=" * 70)
    ...
    try:
        # Отправляем запрос на обновление FAQ
        logger.info("Отправляем запрос на загрузку и индексацию...")
        ...
        async with httpx.AsyncClient(timeout=600.0) as client:
            ...
            params = {
                ...
                "mode": "replace",
                "source_path": FAQ_SOURCE_PATH
            }
            
            start_time = time.time()
            try:
                response = await client.get(
                    f"{MCP_SERVER_URL}/faq/update",
                    params=params,
                    headers=headers
                )
            except httpx.TimeoutException as e:
                logger.error(f"✗ Timeout при индексации (может быть нормально): {e}")
            ...
            if response.status_code == 200:
                ...
                if response_data.get("success"):
                    ...
                    # Получаем статус faq полный логер событий находится в status
                    status = response_data.get("status", {})
                    result["details"]["faq_status"] = status
                    metadata = status.get('metadata', {})
                    logger.info(f"  Статус FAQ: {str(metadata.get('index_status'))}")
                    logger.info(f"  Вопросов в индексе: {status.get('documents_count')}")
                else:
        ...
        logger.error("✗ Timeout при индексации (может быть нормально для больших БЗ)")
        result["details"]["error"] = "Timeout - indexing may still be in progress"
        result["status"] = "TIMEOUT"
...
async def test_faq_indexing_s3() -> Dict[str, Any]:
    ...
    """
    logger.info("\n" + "=" * 70)
    logger.info("3. ИНДЕКСАЦИЯ БЗ S3: ЗАГРУЗКА И ИНДЕКСИРОВАНИЕ ДОКУМЕНТОВ ИЗ S3")
    logger.info("=" * 70)
    
    result = {
        "name": "FAQ Indexing from S3",
        "status": "FAILED",
        "details": {},
        "timestamp": datetime.now().isoformat()
    }
    ...
    try:
        ...
        if not S3_ACCESS_KEY or not S3_SECRET_KEY:
            logger.error("✗ S3_ACCESS_KEY или S3_SECRET_KEY не заданы в переменных окружения")
            result['details']['error'] = "Missing S3 credentials in environment variables"
            test_results["tests"]["faq_indexing_s3"] = result
            return result
        
        logger.info("S3 конфигурация:")
...
Сейчас твоя задача:
- решить, нужно ли вызывать инструмент faq_search. Если нужно вызывать его с коллекцией то только collection=`faq_collection` 
...
...
        ...
        test_query = "Почему при завершении договора я не могу получить выплату в валюте?"
    ...
    test_queries = [
        {
            "name": "Доходность",
            "query": "Что такое ДИД?",
            "expected_keywords": ["часть", "доход", "период"]
        },
        {
            "name": "Налоговый вычет",
            "query": "Документы в страховой компании для налогового вычета?",
            "expected_keywords": ["Справка", "Договор", "Лицензия"]
        },
        {
            "name": "Прямой вопрос из базы знаний",
            "query": "Возможен ли возврат страховки АльфаСтрахование-Жизнь при досрочном погашении кредита?",
            "expected_keywords": ["возврат", "подлежит", "8-800-333-84-48"]
        }
    ]
...
Action Input: входные данные для действия. Если инструмент принимает аргумент 'query', передай его в формате JSON, например: {{"query": "твой запрос", "collection": "faq_collection"}}
...
Если нужно вызывать его с коллекцией то только collection=`faq_collection`
Начинай!
