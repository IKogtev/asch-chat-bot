...
| Phase 5 | Implemented | Deterministic displayed-list selection, advisor-to-`product_info` card/kit handoff, complete identity transfer, invalid-selection clarification, context replacement/reset, and legacy Phase 4 context compatibility | Live multi-turn validation of the current next-action question |

Phases 6–7 remain planned. The `ADVISOR_ENABLED` pilot flag, advisor-specific observability, deployment validation, and business release approval are not part of the current Phase 1–5 implementation.
...
| `agent/rootagent.py` | Phase 4 core integration and Phase 5 deterministic numbered-list resolution, advisor-to-product context transfer, card/kit handoff, invalid-selection clarification, and context reset are implemented |
...
The current recommendation format ends with this next-action question:

`Введи номер продукта, если хочешь посмотреть карточку или напиши если надо скачать комплект какого-то продукта.`

Implement the follow-up behavior promised by that question without changing the validated recommendation format:

1. Persist the displayed `products` list in its validated order so its one-based list numbers remain the source of truth for the next turn.
2. Resolve a standalone displayed number, "1", "второй", "третий" a product code, or an explicit product number NNNN or name against that saved list. Do not infer an ordinal when no valid recommendation context exists.
3. Treat a standalone valid list number as a request for that product's card, matching the wording of the current next-action question.
4. Resolve card verbs to `product_card` and document/kit verbs to `product_kit`, then hand off to the existing `product_info` route instead of generating either result in advisor code.
5. Populate the existing product context with the selected advisor product's complete validated identity before the handoff, preserving `code`, `name`, and `is_active`.
6. Keep recommendation explanations, recommendation comparisons, and client-profile corrections in the advisor route. A product-card or product-kit request is the only Phase 5 handoff covered here.
7. Reject zero, negative, non-numeric, or out-of-range list selections with one concise clarification that states the available displayed numbers. Never silently choose another product.
8. Reset the saved numbered-list context when a new client or a new recommendation session starts, and replace it atomically after each successful recommendation.
9. Add multi-turn tests for number, ordinal word, code, exact name, card verb, kit verb, missing context, out-of-range selection, context replacement/reset, and profile-correction routing.
10. Prove that the existing product-kit `_bot_action` is generated only by `product_info`, not by advisor code.

Exit criterion: every next action offered by the current recommendation message resolves the intended displayed product, card and kit requests preserve existing `product_info` behavior, advisor-only follow-ups stay in `advisor`, and invalid selections produce a deterministic clarification.

Implementation status (September 3, 2026): implemented in `agent/rootagent.py`. The validated displayed product identities are persisted in their exact order, and a deterministic pre-dispatch resolver handles standalone numbers, supported ordinal words, unique codes, and exact product names. A valid selection populates the existing product context with `code + name + is_active` and routes card or kit actions through `product_info`; advisor code never creates `_bot_action`. Missing, ambiguous, non-numeric, and out-of-range selections return a deterministic clarification. Explanation, comparison, and profile-correction messages remain available to the advisor dispatcher path. New-client requests reset the previous advisor profile and numbered list, new-recommendation requests invalidate the previous result list before recalculation, and each successful recommendation atomically replaces `displayed_products`. Legacy Phase 4 contexts remain readable through their validated `top_products`. The formatter fallback now preserves the same next-action question as the main prompt. Focused Phase 1–5 tests pass (`225 passed`); live multi-turn DBHub/LLM validation remains pending.
