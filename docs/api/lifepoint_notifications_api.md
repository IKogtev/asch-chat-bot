# API интеграции ИС "Настя" с LifePoint

## 1. Назначение

API предназначено для интеграции LifePoint (ЛП) с ИС "Настя" для отправки персональных уведомлений пользователям
в поддерживаемые каналы коммуникации.

Основной сценарий:

1. ИС "Настя" формирует уникальный `global_user_id` для каждого пользователя.
...
4. LifePoint отправляет запрос в API ИС "Настя", передавая:
   - `global_user_id` — идентификатор пользователя;
   - `channel` — канал отправки; Не обязательно
   - `message` — текст уведомления.
5. ИС "Настя" по `global_user_id` определяет связанные аккаунты пользователя.
6. ИС "Настя" отправляет уведомление в указанный канал или во все
   доступные каналы пользователя например если не указан channel.

При отправке уведомления LifePoint не передает в API ИС "Настя"
...
LifePoint взаимодействует только с API ИС "Настя".
...
```
Полученный access_token передается во всех последующих запросах к API ИС "Настя" в HTTP-заголовке:
```http
...
зарегистрированном в ИС "Настя", на основе его id, выдается информация о заблокированности пользователя, о его группах и где у данного пользователя есть аккаунты. 
...
---|---|---|---
global_user_id|UUID|Да|Уникальный идентификатор пользователя ИС "Настя"
...
|Поле|Тип|Описание|
|---|---|---|
| global_user_id|UUID |	Уникальный идентификатор пользователя ИС "Настя" |
...
|---|---|---|---|
| global_user_id|UUID |	Уникальный идентификатор пользователя ИС "Настя" |да
...
После получения запроса ИС "Настя" выполняет следующие действия:
...
в ИС "Настя", по которому она самостоятельно определяет
