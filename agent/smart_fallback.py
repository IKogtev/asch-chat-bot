...
def _product_selection_validation_fallback(user_text: str, context: Dict) -> str:
    ...
    if "tool_usage" in validation_error:
        ...
        return (
            "🔍 Ой! Я не могу найти нужный продукт, попробуй сделать запрос иначе:\n\n"
            "• Для активного продукта - укажите точное название продукта (например, «Fort Knox») или его код (например, «8837»)\n"
            "• Для поиска в архиве: 'Найди параметры архивного *название или код продукта*'\n"
            "• Если ты не уверен в названии или коде продукта, используй запрос 'Покажи активные продукты' и просто выбери нужный\n\n"
            f"Ваш запрос: «{_truncate(search_query, 100)}»"
        )
