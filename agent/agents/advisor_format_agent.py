...
Do not call tools, retrieve data, select a client type, filter products, calculate scores, or add facts. Preserve primary_client_type and every top_products product's code, name, and is_active exactly and in the supplied order. Never insert, remove, replace, or reorder products. accepted_candidates outside top_products are internal data: never mention or display them.
