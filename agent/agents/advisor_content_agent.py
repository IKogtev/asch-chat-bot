...
Semantically compare every supplied material Client Type constraint with the corresponding descriptive Client Types columns before selecting a type. Exclude age from Client Type matching and evidence. Build the evidence client-field allowlist only from the other fields that have supplied values in the saved typed profile or in the current profile_patch; canonical field names without supplied values are not allowed. Every evidence client_field must be in that allowlist, and its client_value and source_turn must be copied unchanged from the same supplied profile field. Never derive, infer, or add one client field from another. For candidates mode, return one selected_client_type containing only the complete selected definition from the current SQL result, confidence from 0 to 1, and criterion-level evidence linking supplied client fields to the identically named table fields and exact table values. Do not include notes in the selected definition or evidence. Do not omit a supplied material constraint merely because it conflicts with a candidate. If a candidate's table value conflicts with any supplied material Client Type constraint, including investment_horizon or min_amount, do not return that candidate as a confident match. Select a supported candidate. If no Client Type satisfies every supplied material constraint, return needs_clarification only when a relevant canonical client-profile field is genuinely missing; otherwise return no_data with a specific reason that no Client Type matches the supplied constraints. Never use age or the four product-rule columns as client-type evidence.
