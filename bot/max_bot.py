...
from bot.services.utils import get_document_id, render_results, markdown_to_safe_html, run_http_server, build_universal_menu, handle_show_more, handle_show_all, handle_download_by_ranks
...
from bot.services.handlers import sync_user_profile_to_adk, get_tree_cached, get_authenticated_user, register_handlers
...
#         """Обработчик /start"""
#         user = await get_authenticated_user(event, subscriber_store, "max")
#         if not user:
#             return 
...
#         tree = await get_tree_cached()
#         menu = build_universal_menu(tree, path_list, "max")
...
#                 ok = await handle_show_more(event=event, store=store, user_id=user_id, session_id=session_id, turn_id=turn_id, start_time=start_time, platform="max")
#                 if not ok:
...
#                 ok = await handle_show_all(event, store, user_id, session_id, turn_id, start_time, "max")
#                 if not ok:
#                     response_time = int((time.time() - start_time) * 1000)
#                     answer = "Нет сохранённого списка документов. Сначала найдите файлы по запросу."
#                     await event.message.answer(answer)
...
#                     event, store, doc_handler, user_id, session_id, dl_ranks, turn_id, start_time, "max"
#                 )
#                 await store.append(user_id, "user", user_text)
...
async def main():
    ...
    max_token = os.getenv("MAX_BOT_TOKEN", "REMOVED_SECRET").strip()
    if not max_token:
