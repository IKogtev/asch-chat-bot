...
def register_handlers(dp: Dispatcher, store, subscriber_store, adk, doc_handler, TITLE_START) -> None:
    ...
    async def on_text(m: Message) -> None:
        ...
        try:
            ...
            # --- Синхронизация актуальных пользовательских данных с ADK ---
            # Важно: Только здесь отправляем свежие атрибуты пользователя (без номера телефона) в ADK.
            user_data = await subscriber_store.get_user_data(int(user_id))
            if user_data:
                user_data.pop("phone_number", None)  # Безопасность: не передаем номер телефона
